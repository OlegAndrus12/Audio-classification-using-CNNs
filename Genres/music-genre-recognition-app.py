...
st.write(
    "On the backend of this Web App a Convolutional Neural Network Model is used.The Model was trained on Mel Spectrogram of Music Files in the GTZAN Dataset."
)
st.markdown(
    f"""
        <style>
...
file = st.sidebar.file_uploader(
    "Please Upload Mp3 Audio File Here or Use Demo Of App Below using Preloaded Music",
    type=["mp3"],
)   
...
def show_output(songname):
    ...
    image_data = load_img(
        "melspectrogram.png", color_mode="rgba", target_size=(288, 432)
    )
    ...
    spec_or_prob = st.sidebar.radio(
        "Select one of Below", ("Probability Distribution", "Mel Spectrogram"), key = "radio0"
    )
...
if demo:
    ...
    song = st.sidebar.radio(
        "Which Song you Want to check?",
        (
    ...
    if song == "Bob Marley - War":
        show_output("Bob Marley - War")
...
if file is None:
    ...
    spec_or_prob = st.sidebar.radio(
        "Select one of Below", ("Probability Distribution", "Mel Spectrogram"), key = "radio1"
    )
